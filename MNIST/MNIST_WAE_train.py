...
wae = WAE(784, [784, 800],[800, 800], z_dim, u, v, lamb)
for epoch in range(nepoch):
    wae.optimizer = tf.keras.optimizers.Adam(learning_rate=0.001*0.995**epoch)
    ...
    train_wae(wae, shuffled_inputs, batch_size)

# wae.save_weights(filepath ='trained_models/MNIST_'+str(z_dim)+'Wae.h5')

#random samples
#temp_logits = np.random.normal(loc=0, scale = 1, size = (100, z_dim))
#temp_logits = tf.convert_to_tensor(temp_logits)
#logits, output_images = vae.decoder(temp_logits)
...
#plt.savefig('test_img/Wae_random_samples_'+str(z_dim)+'.png', dpi=500)
